...
import nabd2tetris.jack.compiler.CompilationEngine;
		...
		for (String jackPath : jackFilePathList) {
			...
			while (true) {
				...
				if (Const.TOKEN_TYPE.KEYWORD == token.getTokenType()) {
					...
					addTokenTag(tokenSb, token.getTokenType().getString(), token.getSymbol());
				}
			}
			...
			// コンパイル
			String compiledFilePath = jackPath.substring(0, point) + ".xml";
			CompilationEngine compilationEngine = new CompilationEngine(tokeList, compiledFilePath);
			try {
